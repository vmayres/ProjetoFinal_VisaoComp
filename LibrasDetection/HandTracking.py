...
original_dataset_dir = r"D:\libras-alunos"
mask_dataset_dir = r"D:\libras-mask-alunos"
        ...
        for file in files:
            if file.endswith('.mp4'):  # Apenas arquivos de vídeo
                ...
                while cap.isOpened():
                    ...
                    # Desenhar as marcas do MediaPipe Hands
                    if hand_results.multi_hand_landmarks:
                        for hand_landmarks in hand_results.multi_hand_landmarks:
                            mp_drawing.draw_landmarks(mask_frame, hand_landmarks, mp_hands.HAND_CONNECTIONS)

                    # Desenhar as marcas do MediaPipe Pose
                    if pose_results.pose_landmarks:
                        mp_drawing.draw_landmarks(mask_frame, pose_results.pose_landmarks, mp_pose.POSE_CONNECTIONS)

                    # Aplicar zoom no centro da imagem
                    zoomed_frame = apply_zoom(mask_frame, zoom_factor=1.2)
                    # Deslocar a imagem para a esquerda
                    shifted_left_frame = shift_image(zoomed_frame, shift_pixels=-int(frame_width * 0.1))
                    # Deslocar a imagem para a direita
                    shifted_right_frame = shift_image(zoomed_frame, shift_pixels=int(frame_width * 0.1))
